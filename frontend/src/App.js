...
// ✅ NEW: Importing icons for the advanced UI
import {
  FiCreditCard, FiSend, FiCheckCircle, FiPlusCircle, FiSearch, FiKey,
  FiFileText, FiArrowUpRight, FiArrowDownLeft, FiXCircle, FiCopy, FiPocket, FiLink2
} from 'react-icons/fi';
...
function App() {
  ...
  const sendTokens = async () => { if (!walletData?.privateKey) return alert("❌ Wallet not loaded"); if (!recipient || !amount) return alert("Please fill in a valid recipient and amount."); try { setLoading(true); const res = await axios.post("http://localhost:5000/api/send", { privateKey: walletData.privateKey, to: recipient, amount, token, }); setTxHash(res.data.txHash); alert(`✅ Sent! TX Hash:\n${res.data.txHash}`); if (showHistory) { fetchHistory(walletData.address); } } catch (err) { alert(`❌ Error sending tokens: ${err.response?.data?.error || err.message}`); console.error(err); } finally { setLoading(false); } };
  const verifyTx = async () => { if (!txHash) return alert("Please enter a transaction hash to verify."); try { setLoading(true); const res = await axios.post("http://localhost:5000/api/verify", { txHash }); setVerifyData(res.data); if (showHistory) { fetchHistory(walletData.address); } } catch (err) { alert(`❌ Invalid TX hash: ${err.response?.data?.error || err.message}`); console.error(err); } finally { setLoading(false); } };
  const fetchHistory = async (address) => { try { setLoading(true); const res = await axios.get(`http://localhost:5000/api/history/${address}`); setTransactionHistory(res.data); setShowHistory(true); } catch (err) { alert(`❌ Could not fetch transaction history: ${err.response?.data?.error || err.message}`); console.error(err); } finally { setLoading(false); } };
            ...
            <div style={styles.card}>
              ...
              {verifyData && (
                <div style={styles.verifyContainer}>
                  ...
                  <p style={styles.detailText}><strong>Status:</strong> <span style={{fontWeight: 'bold', color: verifyData.status === 'Success' ? '#10B981' : '#EF4444'}}>{verifyData.status}</span></p>
                  {/* ✅ EDIT: Added BscScan link to verification */}
                  <p style={styles.detailText}>
                    <strong>Explorer:</strong>{" "}
                    <a href={`https://testnet.bscscan.com/tx/${txHash}`} target="_blank" rel="noopener noreferrer" style={styles.link}>
                      View on BscScan <FiLink2 size={12}/>
                    </a>
                  </p>
                </div>
              )}
                        ...
                        {transactionHistory.map((tx) => {
                          ...
                          return (
                            <div key={tx._id} style={styles.historyCard}>
                              <div style={styles.historyIcon(isSent)}>{isSent ? <FiArrowUpRight /> : <FiArrowDownLeft />}</div>
                              <div style={styles.historyDetails}>
                                ...
                                <p style={styles.historyText}><strong>Status:</strong> {tx.status}</p>
                                <p style={styles.historyText}>{new Date(tx.createdAt).toLocaleString()}</p>
                              </div>
                               {/* ✅ EDIT: Corrected BscScan link in history */}
                               <a href={`https://testnet.bscscan.com/tx/${tx.txHash}`} target="_blank" rel="noopener noreferrer" style={styles.link}>
                                <FiLink2 size={14} />
                               </a>
                            </div>
                          );
...
const DetailRow = ({ icon, label, value, onCopy }) => (
  <div style={styles.detailRow}>
    ...
    <div style={styles.detailValue}>
      <span>{value}</span>
      <button onClick={onCopy} style={styles.copyButton}><FiCopy /></button>
    </div>
  </div>
);
...
const styles = {
  ...
  historyIcon: (isSent) => ({ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '36px', height: '36px', borderRadius: '50%', color: '#fff', background: isSent ? "#EF4444" : "#10B981" }),
  ...
  link: { color: '#6B7280', textDecoration: 'none', fontWeight: '500' },
